...
async def main():
    ...
    try:
        ...
        if RABBITMQ_HOST:
            ...
            # Start consumer in background - it handles its own lifecycle
        ...
        # 4. Start HTTP server (optional, for testing)
        http_task = None
        if ENABLE_HTTP_API:
        ...
        # 5. Service is ready - continues even if RabbitMQ is down
        logger.info("=" * 70)
        logger.info("✅ Service ready!")
        if RABBITMQ_HOST:
            logger.info(f"🔌 RabbitMQ: Consuming from {REQUEST_QUEUE} (with auto-reconnect)")
        else:
            logger.info("⚠️ RabbitMQ: Disabled")
        if ENABLE_HTTP_API:
        ...
        if http_task:
            ...
            await consumer_task  # Consumer runs forever
        else:
            # Nothing to run - keep process alive
        ...
        # Don't sys.exit() - let HTTP continue if possible
